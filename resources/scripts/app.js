...
if (pathName.includes("/admin/pages/create")) {
    ...
    nameInputEl.addEventListener("keyup", () => {
        ...
        const newSlug = helpers.createSlug(pageName);
...
if (alertEls) {
    alertEls.forEach(alertEl =>
        alertEl.querySelector(".alert__close").addEventListener("click", e => {
...
const currentTimeString = `${currentDate.getHours()}:${
        ...
        this.openMediaPopup = element => {
            ...
            const subscription = this.mediaPopupVM().selectedMediaElement.subscribe(
                () => {
                    ...
                    if (path) {
                        document
                            .querySelector(".user-img-preview")
...
if (openSubBoxEls) {
    const toggleSubBox = e => {
        ...
        const height = subBox.querySelector(".admin-box__sub-content")
            .clientHeight;
        const scrollHeight = subBox.querySelector(".admin-box__sub-content")
            .scrollHeight;

        if (subBox.classList.contains("open")) {
            subBox.querySelector(".admin-box__sub-content").style.height =
                0 + "px";
